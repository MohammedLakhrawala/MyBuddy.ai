  ...
  return (
    <div className='h-[100vh] w-full bg-cover flex justify-center items-center' style={{backgroundImage: `url(${bg})`}}>
      <form className='w-[90%] h-[600px] max-w-[500px] bg-[#0000009d] backdrop:blur shadow-lg shadow-black-950 flex flex-col items-center justify-center gap-[20px] px-[20px]' onSubmit={handlesubmit}>
        ...
        <input type="text" id='name'  placeholder='Enter your Name' className='w-full h-[60px] outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[18px]' required onChange={(e)=> setName(e.target.value)} value={name}/>
        ...
        {err.length>0 && <p className='text-red-500'>{err}</p> }
            <button className='min-w-[150px] h-[60px] bg-white rounded-full text-black text-[20px] font-semibold cursor-pointer'>Sign Up</button>

            <p className='text-white ' >Already have an account?  <span className='font-semibold text-blue-400 cursor-pointer' onClick={()=> {navigate("/signin")}}>Sign In</span></p>
      </form>
    </div>
  )
}
